...
class compSent:
    ...
    def __init__(self):
        ...
        for line in f:
            ...
            float(a_mean_sum),float(a_sd_sum),float(d_mean_sum),float(d_sd_sum))

    def compSentiment_v(self, intweet):
        sentlist=[]
    ...
    def compSentiment_a(self, intweet):
        sentlist=[]
        for i in xrange(1,len(intweet)):
            word=str(intweet[i]).lower()
            if word in self.sent_dict:
                sent=self.sent_dict[word]
                sentlist.insert(i,sent[3],)
        return numpy.mean(sentlist)
        
    def compSentiment_d(self, intweet):
        sentlist=[]
        for i in xrange(1,len(intweet)):
            word=str(intweet[i]).lower()
            if word in self.sent_dict:
                sent=self.sent_dict[word]
                sentlist.insert(i,sent[5],)
        return numpy.mean(sentlist)